...
static sensor_data_t g_data = { .temperature_c = -1000.0f, .wheel_count = 0, .rpm = 0.0f };

extern void sensor_tasks_start(void);
...
void sensor_manager_init(void)
{
    ...
    if (r != ESP_OK) ESP_LOGW(TAG, "max6675_init failed (%d)", r);

    proximity_config_t p = {
        .pin = PROX_PIN,
        .trigger = PROX_TRIGGER,
        .debounce_us = PROX_DEBOUNCE_US
    };
    r = proximity_init(&p);
    if (r != ESP_OK) ESP_LOGW(TAG, "proximity_init failed (%d)", r);
}
...
void sensor_manager_update(float temp_or_nan, uint32_t count, float rpm)
{
    if (xSemaphoreTake(data_mutex, pdMS_TO_TICKS(20))) {
        ...
        if (count != 0) g_data.wheel_count = count;
        if (rpm >= 0.0f) g_data.rpm = rpm;
